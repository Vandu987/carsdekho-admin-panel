...
if (isset($_GET['delete'])) {
    $id = (int)$_GET['delete'];
    $car = $conn->query("SELECT car_image FROM most_searched_cars WHERE id = $id")->fetch_assoc();
    if ($car) {
        if (file_exists('../uploads/cars/' . $car['car_image'])) {
            unlink('../uploads/cars/' . $car['car_image']);
        }
        if ($conn->query("DELETE FROM most_searched_cars WHERE id = $id")) {
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $search_count = (int)$_POST['search_count'];
    ...
    if ($edit_id) {
        $existing = $conn->query("SELECT car_image FROM most_searched_cars WHERE id = $edit_id")->fetch_assoc();
    ...
    if (isset($_FILES['car_image']) && $_FILES['car_image']['error'] === 0) {
        ...
        if (in_array($ext, $allowed)) {
            $new_filename = 'searched_' . time() . '.' . $ext;
    ...
    if (!$error) {
        if ($edit_id) {
            $sql = "UPDATE most_searched_cars SET car_name = ?, car_image = ?, price_range = ?, search_count = ?, is_active = ? WHERE id = ?";
            $stmt = $conn->prepare($sql);
            $stmt->bind_param("sssiii", $car_name, $car_image, $price_range, $search_count, $is_active, $edit_id);
            $msg = 'Car updated successfully!';
        } else {
            if (!$car_image) {
                $error = 'Please upload an image!';
            } else {
                $sql = "INSERT INTO most_searched_cars (car_name, car_image, price_range, search_count, is_active) VALUES (?, ?, ?, ?, ?)";
                $stmt = $conn->prepare($sql);
                $stmt->bind_param("sssii", $car_name, $car_image, $price_range, $search_count, $is_active);
...
if (isset($_GET['edit'])) {
    $id = (int)$_GET['edit'];
    $edit_data = $conn->query("SELECT * FROM most_searched_cars WHERE id = $id")->fetch_assoc();
}

$cars = $conn->query("SELECT * FROM most_searched_cars ORDER BY search_count DESC");
?>

<div class="top-bar">
    <h4><i class="fas fa-search me-2"></i>Most Searched Cars</h4>
...
<div class="row">
            ...
            <div class="card-body">
                <form method="POST" enctype="multipart/form-data">
                    ...
                    <div class="mb-3">
                        ...
                        <input type="text" name="price_range" class="form-control" placeholder="e.g., Rs. 5-10 Lakh" value="<?php echo $edit_data ? htmlspecialchars($edit_data['price_range']) : ''; ?>" required>
                    </div>
                    <div class="mb-3">
                        <label class="form-label">Search Count</label>
                        <input type="number" name="search_count" class="form-control" value="<?php echo $edit_data ? $edit_data['search_count'] : '0'; ?>">
                    </div>
                    <div class="mb-3">
                    ...
                    <?php if ($edit_data): ?>
                        <a href="most-searched-cars.php" class="btn btn-secondary w-100 mt-2">Cancel</a>
                    <?php endif; ?>
                </form>
            </div>
        </div>
    ...
    <div class="col-md-8 mb-4">
        <div class="card">
            <div class="card-header">
                <i class="fas fa-list me-2"></i>All Most Searched Cars
            </div>
            <div class="card-body">
                            ...
                            <?php if ($cars->num_rows > 0): ?>
                                <?php while ($car = $cars->fetch_assoc()): ?>
                                    <tr>
                                        ...
                                        <td><?php echo htmlspecialchars($car['price_range']); ?></td>
                                        <td><?php echo number_format($car['search_count']); ?></td>
                                        <td>
